...
public class Tree{
	...
	try {
		...
		this.insertNode(new TNode(identificationString, f.readInt(), null, null, null));
	    }
	    f.close();
	}
	catch(IOException e) {
	    System.out.println("Error retrieving tree");
	}
    }
    
    //pseudo code for printTree(Tnode start, Tnode end);
    //to print nodes with interval for example printing a tree between nodes 60 to 150
    /* start.getId() < = end.getId()
       if((start is on left subtree of root or start is the root) and
	 (end is on right subtree of root or end is the root))
       else if (end is on left subtree of root)
		new tree with root at root.getLeft()
		print new tree with (start, end)
       else if (start is on right subtree of root)
	       new tree with root at root.getRight()
	       print new tree with (start, end)
       
       // base case
       let p = start
       repeat until p becomes root or null{
	       print p
	       print entire right subtree of p
	       percolate p upward until it becomes either null or larger(equal) than start
       }
       print root
       let p = root.getRight();
       do while p is not null{
	       if p.getId is less/equal than end.getId(){
		    new tree with root at left child of p
		    print entire new tree
		    print p 
		    p becomes right child of p
	       }
	       else {
		    let p be left child of p
	       {
	{
    ...
    */
	...
	if(p != null){
		...
		System.out.print(p + " in level " + level);
		if(p.getParent() == null){
		    System.out.println(" Root");
		}
		else if(p.getParent().getLeft() == p){
		    System.out.println(" Left child " + p.getParent().getId());
		}
		else{
	...
	if(p != null){
	    ...
	    else if ((p.getLeft()!= null && p.getRight()== null) || (p.getRight()!= null && p.getLeft()== null)){
		TNode q = null;
		if(p.getLeft() != null){            
		    q = p.getLeft();
		    //p.setLeft(q);
